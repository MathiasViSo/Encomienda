...
from .querysets import EncomiendaQuerySet # Agrega esta importación arriba
...
class Empleado(models.Model):
    ...
    # Relación Muchos a Muchos: Un empleado gestiona varias rutas y una ruta tiene varios empleados
...
class Encomienda(models.Model):
    ...
    # Identificación con validadores
    ...
    peso_kg = models.DecimalField(
        ...
        validators=[validar_peso_positivo, MinValueValidator(0.01, message='El peso mínimo es 0.01 kg')]
    )
    ...
    def tiene_retraso(self):
        if not self.fecha_entrega_est or self.esta_entregada: return False
    ...
    def cambiar_estado(self, nuevo_estado, empleado, observacion=''):
        ...
        # Guardar en el historial (necesita importación tardía si hay dependencias circulares, pero aquí ya lo tenemos)
        from .models import HistorialEstado
        HistorialEstado.objects.create(
            encomienda=self, estado_anterior=estado_anterior,
            estado_nuevo=nuevo_estado, empleado=empleado, observacion=observacion
        )
        return self
